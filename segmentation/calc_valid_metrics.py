...
#%%
def main(args):
    which_loss = args.loss_function
    
    which_fold = args.fold_name
    which_net = args.network_name
    RESULTS_FOLDER = f"/data/blobfuse/PSMA_PCA_LESIONS_SEGMENTATION/data_resampled_results/\
validation_predictions/{which_net}/{which_fold}/{which_loss}"
    ...
    save_testmetrics_dir = os.path.join(RESULTS_FOLDER, 'valid_metrics', 'fold'+str(fold), network, experiment_code)
    os.makedirs(save_testmetrics_dir, exist_ok=True)
    save_testmetrics_fpath = os.path.join(save_testmetrics_dir, 'validmetrics.csv')
...
if __name__ == "__main__":  
    ...
    parser.add_argument('--loss-function', type=str, default='dicefocal', metavar='lossname',
                        help='name of loss function that was used in training model')
    parser.add_argument('--fold-name', type=str, default='fold0', metavar='foldname',
                        help='name of validation fold')
